...
@synthesize searchQuery, status, results, delegate, useHTTP, shouldReturnLocation;

- (id)initWithDelegate:(id<BSForwardGeocoderDelegate>)aDelegate
{
	if ((self = [super init])) {
		delegate = aDelegate;
        shouldReturnLocation = YES;
	}
	return self;
}
...
- (void)geocodingSucceded{
    if (shouldReturnLocation && [delegate respondsToSelector:@selector(forwardGeocoderFoundLocation:)]) {
...
{
    if (shouldReturnLocation && [delegate respondsToSelector:@selector(forwardGeocoderError::)]) {
...
- (void)startGeocoding
{
	...
	if (version == 2) {
		...
		NSString* mapsUrl = [[NSString alloc] initWithFormat:@"%@://maps.google.com/maps/geo?q=%@&gl=se&output=xml&oe=utf8&region=%@&sensor=false", 
							 useHTTP ? @"http" : @"https", [searchQuery URLEncodedString],COUNTRY];
		
        NSLog(@"Maps url:%@",mapsUrl);
		...
		// Create the url to Googles geocoding API, we want the response to be in XML
		NSString* mapsUrl = [[NSString alloc] initWithFormat:@"%@://maps.google.com/maps/api/geocode/xml?address=%@&region=%@&sensor=false", 
							 useHTTP ? @"http" : @"https", [searchQuery URLEncodedString],COUNTRY];
