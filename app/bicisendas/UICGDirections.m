...
- (void)loadWithStartPoint:(NSString *)startPoint endPoint:(NSString *)endPoint options:(UICGDirectionsOptions *)options {
	/*NSString *result = [googleMapsAPI stringByEvaluatingJavaScriptFromString:
	 [NSString stringWithFormat:@"getLatLgn('%@')", endPoint]];
	NSLog(@"***RESULT: %@", result);*/
...
- (void)clear {
	/*if([googleMapsAPI isLoading])
		[googleMapsAPI stopLoading];*/
}
